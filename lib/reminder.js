...
class Reminder {
    ...
    // channel_nameとauthor_nicknameも取得
    async getWithName() {
        return new Promise((resolve, reject) => {
            this.db.find({}, (err, reminders) => {
                if(err) reject(err)

                resolve(reminders.map(remind => {
                    const channel = this.client.channels.get(remind.channel_id)
                    const author = this.client.users.get(remind.author_id)
                    remind["channel_name"] = channel.name
                    remind["author_name"] = author.username
                    return remind
                }))
            })
        })
